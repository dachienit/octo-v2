...
export class GenericArtifact extends ArtifactElement {
	...
	private openInNewTab() {
		...
		const blob = new Blob([bytes.buffer as ArrayBuffer], { type: this.getMimeType() });
	...
	public getHeaderButtons() {
		return html`
			<div class="flex items-center gap-1">
				<button class="text-xs px-2 py-1 rounded border border-border hover:bg-accent" @click=${() => this.openInNewTab()}>
