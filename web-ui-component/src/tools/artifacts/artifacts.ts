...
export class ArtifactsPanel extends LitElement {
	...
	private getFileType(
		filename: string,
	): "html" | "svg" | "markdown" | "image" | "pdf" | "excel" | "csv" | "docx" | "text" | "generic" {
	...
	public openOrCreateArtifact(filename: string, content: string) {
		if (this._artifacts.has(filename)) {
			...
			this._artifacts.set(filename, { filename, content, createdAt: new Date(), updatedAt: new Date() });
		}
		this._artifacts = new Map(this._artifacts);
		this.getOrCreateArtifactElement(filename, content);
		this.showArtifact(filename);
		this.onOpen?.();
