...
export class AttachmentOverlay extends LitElement {
	...
	private renderFileContent() {
		...
		switch (fileType) {
			...
			case "excel":
				return html`
					<div class="bg-card border border-border text-foreground p-6 w-full h-full max-w-4xl overflow-auto flex items-center justify-center">
		...
		if (
			...
			switch (fileType) {
				...
				case "docx":
					...
					break;
				case "excel":
					// No preview rendering for excel files
					break;
				case "pptx":
